...
import { useAddAttribute, useAttributeById, useUpdateAttribute } from "@/services/attribute.service";
import { useCompany } from "@/services/company.service";
import { toastError, toastSuccess } from "@/utils/toast";
import React, { useEffect } from "react";
import { Controller, SubmitHandler, useForm } from "react-hook-form";
import Select from "react-select";
import { attributeSchema } from "../_attribute.utils/validationSchema";
...
export default function AddAttribute({ id }: { id: any }) {
  //IMPORTS
  const navigate = useNavigate();

  //DATA
  const { data: attribute } = useAttributeById(id, !!id);
  ...
  const onSubmit: SubmitHandler<Inputs> = async (data) => {
    try {
      ...
      console.log(newAttributeData, "sdfdsfsdfsfsdfsdfsdf");

      if (id) {
        const res = await updateAttribute({ attributeId: id, ...data });
        if (res.data?.message) {
          toastSuccess(res?.data?.message);
          setValue("name", "");
        }
      } else {
        const res = await mutateAsync(newAttributeData);
        if (res.data.data) {
          toastSuccess(res.data.message);
          setValue("name", "");
        }
    ...
    <div className="row">
      <div className="col">
        <div className="global_shadow_border global_padding">
          <h5 className="text-maincolor mb-4">Add Attribute</h5>
          <div className="row">
            <div className="col">
